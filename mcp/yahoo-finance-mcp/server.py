...
@yfinance_server.tool(
    name="get_avg_volume",
    description="""Get the average trading volume for a given ticker symbol over a specified period.
Useful for liquidity risk control checks — higher average volume indicates more liquid options.

Args:
    ticker: str
        The ticker symbol of the stock to get average volume for, e.g. "AAPL"
    period: str
        Valid periods: 1d,5d,1mo,3mo,6mo,1y,2y,5y,10y,ytd,max
        Default is "3mo"
""",
)
async def get_avg_volume(ticker: str, period: str = "3mo") -> str:
    """Get average trading volume for a given ticker symbol over a period."""
    company = yf.Ticker(ticker)
    try:
        if company.isin is None:
            return f"Company ticker {ticker} not found."
    ...
    try:
        hist = company.history(period=period, interval="1d")
        if hist.empty:
            return f"No historical data found for {ticker} over period {period}."
        avg_volume = float(hist["Volume"].mean())
        return json.dumps({"ticker": ticker, "period": period, "avg_volume": avg_volume})
    except Exception as e:
        return f"Error: getting average volume for {ticker}: {e}"
